...
if(matchMedia('(pointer:fine)').matches){document.querySelector('.hero').addEventListener('pointermove',e=>{const x=(e.clientX/innerWidth-.5)*12;const y=(e.clientY/innerHeight-.5)*12;document.querySelector('.portrait-shell').style.transform=`translate(${x}px,${y}px)`},{passive:true})}
